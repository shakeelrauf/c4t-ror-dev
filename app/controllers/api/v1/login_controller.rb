  ...
  def token
  	if(!params["client_id"] ||
         ...
         params["grant_type"] != "client_credentials")
  		return render_json_response({:error => MISSING_PARAMS_MSG, :success => true}, :ok)
  	else
  		user =  User.where(isActive: true, access_token: request.authorization).first
  		return render_json_response({:error => MISSING_PARAMS_MSG, :success => true}, :ok) if (!user || !user.isValidPassword(params[:client_secret]))
  		token = {}
